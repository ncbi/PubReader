// $Id: figpopup.js 21813 2014-05-09 20:29:43Z kolotev $
...
(function($j) {
	...
	$j.fn.popupSensor = function  (opts) {
	    ...
	    return this.each(function(i){  
			...
			elm.bind('pointerup', 	__sensorTouchEnd) // handle clicks on touch devices
		    //
		    elm.data('popupSensor.cfg', $j.extend({}, cfg))
		    elm.data('figpopup.flag', true)
		})
	}
	//
	function __sensorClick (e) {//__loginfo('trace: sensor.__sensorClick()')
	    var elm = $j(this), pt = elm.data('pt')

	    // return true
	    // if pointer type is a "mouse"
	    // we want to let user jump on the link behind
	    // popup sensor

	    // return false
	    // if pointer type is different ("touch", "pen") 
	    // we want to prevent jumping to the link to let user
	    // interact with popup

	    return (pt == "mouse" ? true : false)
	}

	//
	function __sensorTouchEnd (e) {//__loginfo('trace: sensor.__sensorTouchEnd()')
	    var elm = $j(this), pt, oe;
	    
	    // prevent dismissal of popup from one of the ancestor elements
	    e.stopPropagation() 
	    
	    // handle pointerup event for touch pointer to emulate popup on touch instead of hover.
	    if (e != null && (oe = e.originalEvent) != null && (pt = oe.pointerType) == "touch") {
		...
		// memorize the pointer type to handle click accordingly
		elm.data('pt', pt)	    
	...
	//
	function __sensorMouseIn (e) {//__loginfo('trace: sensor.__sensorMouseIn()')
	    var elm = $j(this),
		cfg = __getCfg(elm)

	    if (e != null && e.originalEvent != null && e.originalEvent.pointerType == "mouse")
			elm.bind(JQ_MOUSEENTER_EVT, __sensorInstantMouseIn)
		else
			$j(document).trigger('pointerup') // to hide other open canvas(es)
	    ...
	    if (typeof elm.data(SENSOR_EVT_TS) === "undefined" || elm.data(SENSOR_EVT_TS) === null) {
			elm.data(SENSOR_EVT_TS, e.timeStamp)
	...
	//
	function __sensorMouseOut (e) {//__loginfo('trace: sensor.__sensorMouseOut()')
	    var elm 	= $j(this)
	    elm.data('__sensor.mouseIn', false)
	    __evaluateSensorCanvasStates.apply(this, [e])
	}

	//
	function __sensorInstantMouseIn (e) {//__loginfo('trace: sensor.__sensorInstantMouseIn()')
	    var elm = $j(this)
	    elm.data('__sensor.instantMouseIn', true) // set the state
	    elm.bind(JQ_MOUSELEAVE_EVT, __sensorInstantMouseOut)
	}
	//
	function __sensorInstantMouseOut (e) {//__loginfo('trace: sensor.__sensorInstantMouseOut()')
	    var elm = $j(this)
	    ...
	    setTimeout(function(){
		    //__loginfo('trace: sensor.__sensorInstantMouseOut() delayed evaluation')
		     __evaluateSensorCanvasStates.apply(self, [e])
	    }, cfg.delayOut )
	    
	}

    	//
	function __canvasMouseIn (e) {//__loginfo('trace: sensor.__canvasMouseIn()')
	    var elm = $j(this)
	    elm.data('__canvas.mouseIn', true)
	}
    	//
	function __canvasMouseOut (e) {//__loginfo('trace: sensor.__canvasMouseOut()')
	    var elm = $j(this)
	    elm.data('__canvas.mouseIn', false)
	    __evaluateSensorCanvasStates.apply(this, [e])
	}
	
	function __evaluateSensorCanvasStates (e) {//__loginfo('trace: sensor.__evaluateSensorCanvasStates()')
	    var elm 	= $j(this)
	    var canvas 	= __getCanvas(elm)
	...
	//
	function __canvasShown (e) {//__loginfo('trace: sensor.__canvasShown()')
	    var elm = $j(this),
		cfg = __getCfg(elm)
	    
	    $j(document).one('pointerup', function(e) {elm.trigger(SENSOR_MOUT_EVT)})
	    
	    elm.data('__canvas.shown', true)
	}
	//
	function __canvasGone (e) {//__loginfo('trace: sensor.__canvasGone()')
	    var elm = $j(this),
		cfg = __getCfg(elm)
	    ...
	    {
		var lifeSpan = e.timeStamp - elm.data(SENSOR_EVT_TS)
		
		// applog pinger
		if (cfg.statIfLonger < lifeSpan )
		    ncbi.sg.ping(this, e, JSEVENT_NAME, ["lifespan=" +  lifeSpan])
	...
	function __getCanvas (elm) {
	    ...
	    var rid, fig, canvas, cfg
	    
	    rid 	= elm.attr('rid-figpopup') || elm.attr('co-rid') || elm.attr('rid') || elm.closest('[id]').attr('id')
	    if (rid == null)
	    	return null
		fig 	= $j('#' + (rid.replace(/(:|\.)/g, '\\$1').split(' ').join(',#'))).first()
		if (fig == null)
			return null
	...
	*/
	...
	$j.fn.extend(popupCanvas.prototype,  {
		...
		{
		    var self = this
		    this.bind(JQ_MOUSEENTER_EVT, function (e) {self.__in(e)})
		    this.bind(JQ_MOUSELEAVE_EVT, function (e) {self.__out(e)})
		    this.bind(CANVAS_SHOW_EVT, function (e) {self.show(e)})
		    this.bind(CANVAS_HIDE_EVT, function (e) {self.hide(e)})
		}
	    ...
	    //
	    __in: function (e) {//__loginfo('trace: canvas.__in()')
    		var cfg 	= this.__cfg
		var elm  	= cfg.el
		elm.trigger(CANVAS_MIN_EVT)
	    },
	    //
	    __out: function (e) {//__loginfo('trace: canvas.__out()')
		var cfg 	= this.__cfg
		var elm  	= cfg.el
	    ...
	    this.__canvas.bind('pointerup', function(e){
			e.preventDefault();
	    })
		...
		if (typeof el !== "undefined")
		{
		    ...
		    {   //console.info('co-rid is not empty #### %s', sensorEl.attr('co-rid'))
				var coRidClone = $j('#' + sensorEl.attr('co-rid').replace(/(:|\.)/g,'\\$1').split(' ').join(",#")).clone()
		...
		var self = this
		this.__resizer == null
		    ? this.__resizer = $j.debounce(500, function(e){self.__canvasArrangeLocation()})
		    : 0
		
		return this.__resizer
	    },
	    //
	    show: function (e) {//__loginfo('trace: canvas.show()')
    		var cfg 	= this.__cfg,
		    elm  	= cfg.el,
		    self 	= this
			...
			if (typeof $j.debounce == "function")
			    $j(window).bind("resize", this.__getResizeHandler())
			elm.trigger(CANVAS_SHOWN_EVT)

	    },
	    //
	    hide: function (e) {//__loginfo('trace: canvas.hide()')
			var cfg 	= this.__cfg,
			    elm 	= this.__cfg.el,
			    self	= this
			
			if (typeof $j.debounce == "function")
			    $j(window).unbind("resize", this.__getResizeHandler())

			this.__canvas.hide()
			this.__canvas.removeAttr('style')
			elm.trigger(CANVAS_GONE_EVT)
			$j(document).trigger('pointerup')
	    },
	    //
...
//__loginfo('canvas.addImage() onload.handler img ' + im.attr('src') + ' was loaded')
		...
		$l.find('a').replaceWith(function(){ return $j(this).contents() })
		...
		return  this.__canvas.children(".co-inner").length > 0 ? this.__canvas.children(".co-inner") : this.__canvas
	    },
	    //
...
//})
	...
	$j.extend ( config, {
	    ...
	    delayOut:                   DELAYOUT, // not implemented yet, but actually it could be an alias to timeout.
            timeout:       		200,
            over:          		function(e){ $j(this).trigger(SENSOR_MIN_EVT) }, // handler onMouseOver
            out:           		function(e){ $j(this).trigger(SENSOR_MOUT_EVT) } // handler  onMouseOut
        } )
